...
public:
    ...
    int checaOcorrencias(const Data &d);
...
void ListaAux::Adicionar(Noh* novoNoh) {
    if (mPrimeiro == NULL) {
        ...
        atual->mProximo = novoNoh;
    }
    mTamanho++;
}

int ListaAux::checaOcorrencias(const Data &d) {
    Noh *aux;
    aux = mPrimeiro;
    int cont = 0;
    while(aux != NULL) {
        if (aux->mValor.FazAniversarioEm(d)) {
            cont++;
        }
        aux = aux->mProximo;
    }
...
float Hash::PorcentagemDeUso() const {
    return (float)mNroPosicoes / (float)mTamanho;
}

unsigned Hash::QtdeAniversariantes(const Data& d) {
    int posicao = d.Hash(mNroPosicoes);
    return mElementos[posicao].checaOcorrencias(d);
...
int main() {
    ...
    cout << tabela.QtdeAniversariantes(hoje) << ' ' <<
        fixed << setprecision(2) << tabela.PorcentagemDeUso() << endl;
    return 0;
}