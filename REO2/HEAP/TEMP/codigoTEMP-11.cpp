...
void Heap::corrigeSubindo(int i) {
    ...
    int pai = Pai(i);
    if (pai >= 0 && dados[pai].prioridade < dados[i].prioridade) {
...
void Heap::corrigeDescendo(int i) {
    ...
    int maior = i;
    if (esquerdo < tamanho && dados[esquerdo].prioridade > dados[i].prioridade) {
        maior = esquerdo;
    }
    if (direito < tamanho && dados[direito].prioridade > dados[maior].prioridade) {
        maior = direito;
    }
    if (maior != i) {
